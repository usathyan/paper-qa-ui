...
async def process_question_async(question: str, config_name: str = "optimized_ollama") -> Tuple[str, str, str, str]:
    ...
    for attempt in range(max_retries):
        try:
            ...
            # In async contexts, PaperQA returns an asyncio.Task; otherwise an AnswerResponse
            if asyncio.isfuture(result) or isinstance(result, asyncio.Task) or hasattr(result, "__await__"):
                answer_response = await result
            else:

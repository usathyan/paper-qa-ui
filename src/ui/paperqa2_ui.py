    ...
    try:
        ...
        try:
            ...
            try:
                ...
                if (str(settings.llm).lower().startswith('ollama/') or 'ollama' in str(settings.llm).lower()):
        ...
        for i, file_obj in enumerate(files):
            ...
            try:
                ...
                try:
                    ...
                    added_name = await app_state["docs"].aadd(str(dest_path), settings=app_state["settings"])
                    logger.info(f"Indexed {added_name or source_path.name} in {time.time() - t0:.2f}s")
                    if "status_tracker" in app_state:
                    ...
                    failed_files.append(f"{source_path.name}: indexing failed: {str(index_err)}")
                    if "status_tracker" in app_state:
                        app_state["status_tracker"].add_status(f"❌ Failed to index {source_path.name}")
                    continue
...
async def process_question_async(question: str, config_name: str = "optimized_ollama") -> Tuple[str, str, str, str, str]:
        ...
        try:
            ...
            async with app_state["query_lock"]:
                ...
                if app_state.get("docs") is None:
                    ...
                    for d in app_state.get("uploaded_docs", []):
                        try:
                            ...
                            logger.warning(f"Skipping doc that failed to add: {d.get('filename')}: {e}")
                ...
                def _blocking_query() -> object:
                    ...
                    try:
                        result = loop.run_until_complete(app_state["docs"].aquery(question, settings=settings))
                ...
                session = await asyncio.get_running_loop().run_in_executor(None, _blocking_query)
            ...
            if "Event loop is closed" in str(e):
                ...
                try:
                    ...
                    logger.info("Reloaded litellm to reset async HTTP client after loop-close error")
                except Exception:
                    pass
                if attempt < max_retries - 1:
                    ...
                    error_msg = "❌ Internal async client error. Please retry the question."
...
def process_question(question: str, config_name: str = "optimized_ollama") -> Tuple[str, str, str, str, str, str, str]:
    """Synchronous wrapper for process_question_async."""
    answer_html, sources_html, metadata_html, intelligence_html, error_msg = asyncio.run(process_question_async(question, config_name))
    ...
    return answer_html, sources_html, metadata_html, intelligence_html, error_msg, progress_html, status_html
...
def format_sources_html(contexts: List) -> str:
    ...
    for i, context in enumerate(contexts, 1):
        try:
            ...
            # Fallbacks
            display_name = citation or title or getattr(getattr(context, 'text', object()), 'name', None) or f'Source {i}'
            ...
            meta = f" <small style='color:#666'>({' | '.join(meta_bits)})</small>" if meta_bits else ''

            html_parts.append("<div style='margin-bottom: 10px; padding: 10px; background: #fff; border-left: 3px solid #007bff;'>")
    ...
    try:
        ...
        for c in contexts or []:
            try:
                ...
                doc_title = None
                if doc is not None:
                    doc_title = getattr(doc, 'formatted_citation', None) or getattr(doc, 'title', None) or getattr(doc, 'docname', None)
                if not doc_title:
                    doc_title = 'Unknown source'
                txt = ''
                if hasattr(c, 'text'):
                    t = getattr(c.text, 'text', None)
                    txt = t if isinstance(t, str) else (str(c.text) if c.text is not None else '')
                else:
                    txt = str(c)
            ...
            for j in range(i + 1, len(docs_list)):
                ...
                for w1, w2 in antonym_pairs:
                    if w1 in text_a and w2 in text_b:
                        conflict_items.append(f"{doc_a} mentions '{w1}', while {doc_b} mentions '{w2}'.")
                    if w2 in text_a and w1 in text_b:
                        conflict_items.append(f"{doc_a} mentions '{w2}', while {doc_b} mentions '{w1}'.")
        ...
        if answer:
            for sent in (answer.replace('\n', ' ').split('. ')):
                ...
                if not s:
                    continue
                if any(k in s.lower() for k in ["suggest", "indicat", "demonstrat", "evidence", "supports", "contradict"]):
                    insights.append(s)
                if len(insights) >= 5:
                    break
        if not insights:
            ...
            for c in contexts[:5]:
                try:
                    ...
                    if snippet:
                        insights.append(snippet[:160] + ('...' if len(snippet) > 160 else ''))
                except Exception:
                    continue

        # Evidence summary: count by doc
        ev_summary_items = [f"{doc}: {len(chunks)} excerpt(s)" for doc, chunks in by_doc.items()]
        ...
        for c in contexts or []:
            try:
                ...
                scored_items.append((score if isinstance(score, (int, float)) else None, display, page, snippet))
            except Exception:
                continue
        # Sort by score desc, None at end
        scored_items.sort(key=lambda x: (-x[0], 1) if isinstance(x[0], (int, float)) else (9999, 1))

        parts = ["<div style='background:#fff; padding:12px; border-radius:6px;'>",
        ...
        parts.append("<div style='margin-top:8px'><strong>Evidence summary</strong><ul>")
        if ev_summary_items:
        ...
        if scored_items:
            parts.append("<div style='margin-top:8px'><strong>Top evidence (by score)</strong>")
            parts.append("<div style='overflow-x:auto'><table style='width:100%; border-collapse:collapse'>")
            parts.append("<tr><th style='text-align:left;padding:6px;border-bottom:1px solid #ddd'>Source</th>"
...
with gr.Blocks(title="Paper-QA UI", theme=gr.themes.Soft()) as demo:
    ...
    ask_button.click(
        ...
        outputs=[answer_display, sources_display, metadata_display, intelligence_display, error_display, status_display, status_display]
    )
